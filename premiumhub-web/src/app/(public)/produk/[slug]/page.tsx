...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    if (params.slug) {
      productService.getBySlug(params.slug as string)
        .then(res => {
          if (res.success) {
            setProduct(res.data)
  ...
  if (loading) {
    return (
      <><Navbar />
        <div className="max-w-4xl mx-auto px-4 py-16"><div className="h-96 bg-white/50 rounded-2xl animate-pulse" /></div>
        <Footer />
      </>
    )
  }
  ...
  if (!product) {
    return (
      <><Navbar />
        <div className="text-center py-32"><h1 className="text-2xl font-bold">Produk tidak ditemukan</h1></div>
        <Footer />
      </>
    )
  }
