...
func TestDigiConnectCheckoutWithWalletSmoke(t *testing.T) {
	svc, db, userID := setupDigiConnectCheckoutSmoke(t, 100000)

	entitlement, err := svc.CheckoutWithWallet(userID, DigiConnectCheckoutInput{PlanCode: "digiconnect_starter"})
	if err != nil {
		t.Fatalf("checkout: %v", err)
	}
	if entitlement.PlanCode != "digiconnect_starter" || entitlement.Status != "active" || entitlement.Price != 49000 {
	...
	if user.WalletBalance != 51000 {
		t.Fatalf("expected wallet balance 51000, got %d", user.WalletBalance)
	}
	...
	if ledger.Type != "debit" || ledger.Amount != 49000 || ledger.BalanceAfter != 51000 {
...
func TestDigiConnectCheckoutWithWalletRejectsInsufficientBalance(t *testing.T) {
	...
	if _, err := svc.CheckoutWithWallet(userID, DigiConnectCheckoutInput{PlanCode: "digiconnect_starter"}); err == nil {
