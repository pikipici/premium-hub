...
func Setup(db *gorm.DB, cfg *config.Config) *gin.Engine {
	...
	siteBannerRepo := repository.NewSiteBannerRepo(db)
	siteHeroBgRepo := repository.NewSiteHeroBgRepo(db)
	sosmedHeroSlideRepo := repository.NewSosmedHeroSlideRepo(db)
	...
	siteBannerSvc := service.NewSiteBannerService(siteBannerRepo)
	siteHeroBgSvc := service.NewSiteHeroBgService(siteHeroBgRepo)
	sosmedHeroSlideSvc := service.NewSosmedHeroSlideService(sosmedHeroSlideRepo)
	...
	sosmedHeroSlideHandler := handler.NewSosmedHeroSlideHandler(sosmedHeroSlideSvc)
	sosmedHeroSlideHandler.SetBannerAssetStorage(bannerAssetStorage)
	...
	api.GET("/public/flash-sale", siteFlashSaleHandler.PublicActive)
	...
	admin.POST("/banners/upload-image", siteBannerHandler.UploadImage)
	admin.PUT("/banners/:id", siteBannerHandler.AdminUpdate)
	...
	admin.GET("/sosmed-hero", sosmedHeroSlideHandler.AdminList)
	admin.POST("/sosmed-hero", sosmedHeroSlideHandler.AdminCreate)
	admin.PUT("/sosmed-hero/:id", sosmedHeroSlideHandler.AdminUpdate)
	admin.DELETE("/sosmed-hero/:id", sosmedHeroSlideHandler.AdminDelete)
	admin.POST("/sosmed-hero/upload-image", sosmedHeroSlideHandler.AdminUploadImage)
	...
	admin.DELETE("/flash-sale/:id", siteFlashSaleHandler.AdminDelete)
